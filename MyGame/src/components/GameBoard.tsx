...
const GameBoard: React.FC<GameBoardProps> = ({ puzzle, onCorrect, onIncorrect, onTimeUp, isLoading, timeLimit, gameMode = 'banana' }) => {
    ...
    return (
        <div className="w-full max-w-6xl bg-white/95 backdrop-blur-xl rounded-[2rem] sm:rounded-[3rem] border-4 sm:border-8 border-candy-yellow shadow-xl p-3 sm:p-6 md:p-8 relative overflow-hidden">
            {/* Timer Progress Bar */}
            ...
            <AnimatePresence>
                {isLoading && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="absolute inset-0 bg-white/80 z-20 flex flex-col items-center justify-center p-4 border-4 border-candy-yellow/20 m-2 rounded-[1.5rem]"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-8 items-center">
                ...
                <div className="relative">
                    <motion.div
                        layout
                        className={`relative w-full rounded-[1.5rem] sm:rounded-[2.5rem] overflow-hidden border-4 border-candy-yellow/10 bg-gradient-to-br from-gray-50/50 to-white flex items-center justify-center p-4 sm:p-8 shadow-inner ${isMath ? 'min-h-[220px] lg:min-h-[320px]' : 'aspect-video max-h-[30vh] lg:max-h-[45vh]'}`}
                    >
                    ...
                    <div className="absolute -top-4 -right-4 flex items-center justify-center">
                        <div className="relative w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full border-4 border-candy-yellow shadow-lg flex items-center justify-center">
                            <Timer size={18} className={`absolute top-2 sm:top-3 sm:size-8 ${timerColor}`} />
                            <span className={`text-xl sm:text-3xl font-black mt-3 sm:mt-5 ${timerColor}`}>
                                {timeLeft}
                            </span>
                        </div>
                    </div>
                ...
                <div className="flex flex-col space-y-4">
                    ...
                    <div className="space-y-4">
                        ...
                        {isMath ? (
                            ...
                            <div className="bg-gray-50 rounded-[1.5rem] p-6 sm:p-8 border-4 border-gray-100 shadow-inner text-center relative overflow-hidden group">
                        ...
                        <div className="flex space-x-2">
                            ...
                            <motion.button
                                ...
                                className={`candy-button flex flex-1 items-center justify-center gap-3 py-4 sm:py-6 rounded-[1.5rem] text-xl font-black shadow-[0_6px_0_0_#ad1457] border-4 border-white ${!answer || isChecking || timeLeft === 0 ? 'bg-gray-300 shadow-none grayscale cursor-not-allowed' : 'bg-candy-pink text-white'}`}
                            >
